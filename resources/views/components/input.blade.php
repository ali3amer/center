...
<div  class=" flex flex-wrap px-3 w-{{ $width }}">
    ...
    <input type="{{ $type }}" wire:model="{{ $name }}" placeholder="{{$placeholder ?? $label}}" name="{{ $name }}" id="{{ $name }}" class="appearance-none text-center block w-full text-gray-700 border rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white" {{ $attributes }} />
