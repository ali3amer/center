...
class Settings extends Component
{
    ...
    public function getFromExcel()
    {
        ...
        for ($i = 1; $i <= 1283; $i++) {
            ...
            $trainer = \App\Models\Trainer::where('arabic_name', trim($row[6]))->first();

            if ($trainer == null) {
                $trainer = \App\Models\Trainer::create([
                    'arabic_name' => trim($row[6]),
                    'gender' => $row[7],
                    'user_id' => auth()->id(),
                ]);
            }

            $course = \App\Models\Course::where('arabic_name', trim($row[4]))->first();

            if ($course == null) {
                $course = \App\Models\Course::create([
                    'arabic_name' => trim($row[4]),
                    'type' => $type,
                    'duration' => 'day',
                    'duration_value' => 5,
                    'user_id' => auth()->id(),
            ...
            $batch = \App\Models\Batch::where('course_id', $course->id)->where('start_date', $date)->first();
            ...
            $student = \App\Models\Student::where('arabic_name', trim($row[3]))->first();
            if ($student == null) {
                $student = \App\Models\Student::create([
                    'id' => $row[1],
                    'arabic_name' => trim($row[3]),
                    'gender' => 'male',
                    'user_id' => auth()->id(),
                ]);
            }
            ...
            \App\Models\BatchStudent::create([
                ...
                'batch_id' => $batch->id,
                'want_certification' => $row[2] != 0 ? true : false,
                'certification_id' => $row[2] != 0 ? $row[2] : null,
                'date' => $date,
                'user_id' => auth()->id(),
            ]);
        }
